...
This function passes through `MeetingProvider` and `DevicesProvider` and will be triggered in `AudioInputProvider`. 
It determines how to deal with the current audio input device when audio put devices change and the current audio input device is the default device.
You can implement your own audio input transform device with this function.
